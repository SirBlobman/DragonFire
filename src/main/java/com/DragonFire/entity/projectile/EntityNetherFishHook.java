...
public class EntityNetherFishHook extends EntityFishHook {
    private EntityNetherFishHook.State currentState = EntityNetherFishHook.State.FLYING;
    ...
    public EntityNetherFishHook(World worldIn) {
        super(worldIn, worldIn.isRemote ? FMLClientHandler.instance().getClientPlayerEntity() : null);
    }

    public EntityNetherFishHook(World worldIn, EntityPlayer fishingPlayer)
    {
    ...
    public void onUpdate() {
        ...
        if (this.getAngler() == null) {
            ...
            if (this.currentState == EntityNetherFishHook.State.FLYING) {
                if (this.caughtEntity != null) {
                    this.motionX = 0.0D;
                    this.motionY = 0.0D;
                    this.motionZ = 0.0D;
                    this.currentState = EntityNetherFishHook.State.HOOKED_IN_ENTITY;
                    return;
                }
                ...
                if (f > 0.0F) {
                    ...
                    this.motionY *= 0.2D;
                    this.motionZ *= 0.3D;
                    this.currentState = EntityNetherFishHook.State.BOBBING;
                    return;
                }
                ...
                if (!this.world.isRemote) {
                    try {
                        ...
                        Util.print("Failed to invoke method checkCollision");
                    }
                }
                ...
                if (this.currentState == EntityNetherFishHook.State.HOOKED_IN_ENTITY) {
                    if (this.caughtEntity != null) {
                        if (this.caughtEntity.isDead) {
                            this.caughtEntity = null;
                            this.currentState = EntityNetherFishHook.State.FLYING;
                        } else {
                ...
                if (this.currentState == EntityNetherFishHook.State.BOBBING) {
                    this.motionX *= 0.9D;
                    this.motionZ *= 0.9D;
    ...
    private void catchingFish(BlockPos p_190621_1_) {
        ...
        if (this.rand.nextFloat() < 0.5F && !this.world.canSeeSky(blockpos)
                && this.world.provider.getDimension() == 0) {
            --i;
        }
        ...
        if (this.ticksCatchable > 0) {
            ...
            if (this.ticksCatchableDelay > 0) {
                ...
                if (block == Blocks.WATER || block == Blocks.FLOWING_WATER) {
                    ...
                    if (this.rand.nextFloat() < 0.15F) {
                        worldserver.spawnParticle(EnumParticleTypes.FLAME, d0, d1 - 0.10000000149011612D, d2, 1, f1,
                                0.1D, f2, 0.0D);
                    }
                    float f3 = f1 * 0.04F;
                    float f4 = f2 * 0.04F;
                    worldserver.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d0, d1, d2, 0, f4, 0.01D, (-f3), 1.0D);
                    worldserver.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, d0, d1, d2, 0, (-f4), 0.01D, f3, 1.0D);
                }
            } else {
                ...
                if (block0 == Blocks.WATER || block0 == Blocks.FLOWING_WATER) {
                    ...
                    worldserver.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX, d3, this.posZ,
                            (int) (1.0F + this.width * 20.0F), this.width, 0.0D, this.width, 0.20000000298023224D);
                    ...
                    this.motionY = -0.1F * MathHelper.nextFloat(this.rand, 0.6F, 1.0F);
                    this.playSound(SoundEvents.ENTITY_BOBBER_SPLASH, 0.25F,
                            0.4F + (this.rand.nextFloat() - this.rand.nextFloat()) * 0.2F);
                    worldserver.spawnParticle(EnumParticleTypes.FLAME, this.posX, d3, this.posZ,
                            (int) (1.0F + this.width * 20.0F), this.width, 0.0D, this.width, 0.20000000298023224D);
                    worldserver.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, this.posX, d3, this.posZ,
                            (int) (1.0F + this.width * 20.0F), this.width, 0.0D, this.width, 0.20000000298023224D);
                }
            ...
            if (this.rand.nextFloat() < f5) {
                ...
                if (block1 == Blocks.WATER || block1 == Blocks.FLOWING_WATER) {
                    ...
                    worldserver.spawnParticle(EnumParticleTypes.SMOKE_LARGE, d4, d5, d6, 2 + this.rand.nextInt(2),
                            0.10000000149011612D, 0.0D, 0.10000000149011612D, 0.0D);
                }
            }
    ...
    public int handleHookRetraction() {
        if (!this.world.isRemote && this.getAngler() != null) {
            ...
            if (this.caughtEntity != null) {
                ...
                lootcontext$builder.withLuck(this.luck + this.getAngler().getLuck());
                List<ItemStack> result;
                double d = MathHelper.floor(this.getEntityBoundingBox().minY) + 1.0F;
                Block block = this.world.getBlockState(new BlockPos(this.posX, d - 1.0D, this.posZ)).getBlock();
                ResourceLocation loottable;
                if (block == Blocks.LAVA || block == Blocks.FLOWING_LAVA) {
                    loottable = DFLootTables.NETHER_FISHING;
                } else {
                    loottable = LootTableList.GAMEPLAY_FISHING;
                }
                result = this.world.getLootTableManager().getLootTableFromLocation(loottable)
                ...
                for (ItemStack itemstack : result) {
                    ...
                    entityitem.motionY = d1 * d4 + MathHelper.sqrt(d3) * 0.08D;
                    entityitem.motionZ = d2 * d4;

                    this.world.spawnEntity(entityitem);
                    this.getAngler().world.spawnEntity(new EntityXPOrb(this.getAngler().world, this.getAngler().posX,
                            this.getAngler().posY + 0.5D, this.getAngler().posZ + 0.5D, this.rand.nextInt(6) + 1));
    ...
    private boolean shouldStopFishing() {
        ...
        if (!this.getAngler().isDead && this.getAngler().isEntityAlive() && (flag || flag1)
                && this.getDistanceSq(this.getAngler()) <= 1024.0D) {
            return false;
        } else {
            this.setDead();
            return true;
